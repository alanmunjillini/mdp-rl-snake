...
# Old init_conds
# init_conds = [[5, 5, 2, 2, 18, 10, 1, 1, 4, 5, 2],
#               [5, 5, 2, 2, 18, 10, 3, 4, 4, 5, 2],
#               [5, 5, 2, 2, 10, 18, 3, 4, 4, 5, 2],
#               [5, 2, 2, 2, 10, 18, 5, 5, 4, 5, 2]
#               ]

# New init_conds
init_conds = [[5, 5, 2, 2, 18, 10, 3, 1, 4, 5, 2], # python3 mp10.py --snake_head_x 5 --snake_head_y 5 --food_x 2 --food_y 2 --width 18 --height 10 --rock_x 3 --rock_y 1 --bigfood_x 4 --bigfood_y 5 --bigfood_val 2 
              [5, 5, 2, 2, 18, 10, 3, 3, 4, 5, 2], # python3 mp10.py --snake_head_x 5 --snake_head_y 5 --food_x 2 --food_y 2 --width 18 --height 10 --rock_x 3 --rock_y 3 --bigfood_x 4 --bigfood_y 5 --bigfood_val 2 
              [5, 5, 2, 2, 10, 18, 3, 3, 4, 5, 2]  # python3 mp10.py --snake_head_x 5 --snake_head_y 5 --food_x 2 --food_y 2 --width 10 --height 18 --rock_x 3 --rock_y 3 --bigfood_x 4 --bigfood_y 5 --bigfood_val 2 
            # hidden cases
              ]
...
class Application:
    def __init__(self, args):
        ...
        self.env = SnakeEnv(args.snake_head_x, args.snake_head_y, args.food_x, args.food_y,
                            ...
                            bigfood_x=args.bigfood_x, bigfood_y=args.bigfood_y, bigfood_val=args.bigfood_val)
    ...
    def execute(self):
        if self.args.early_check:
            ...
            # Uncomment these lines and modify diff_dimensions to generate data for different dimensions
            # diff_dimensions = [(18, 17), (20, 19), (21, 18), (30, 30), (32, 37), (28, 33)]
            # for k in range(len(diff_dimensions)):
            ...
            for game in range(1, len(actions_list) + 1):
                ...
                # Uncomment these lines to generate tests for different dimensions
                # Remember to comment out line 127 if you uncomment this one
                # width, height = diff_dimensions[k]
                # self.env = SnakeEnv(self.args.snake_head_x, self.args.snake_head_y, self.args.food_x, self.args.food_y,
                #                     width, height, rock_x=self.args.rock_x, rock_y=self.args.rock_y,
                #                     random_seed=42)


                self.env = SnakeEnv(self.args.snake_head_x, self.args.snake_head_y, self.args.food_x, self.args.food_y,
                                    self.args.width, self.args.height, rock_x=self.args.rock_x, rock_y=self.args.rock_y,
                                    bigfood_x=self.args.bigfood_x, bigfood_y=self.args.bigfood_y, bigfood_val=self.args.bigfood_val,
                                    random_seed=42)
    ...
    def test(self):
        ...
        for i, cond in enumerate(init_conds):
            snake_head_x, snake_head_y, food_x, food_y, width, height, rock_x, rock_y, bigfood_x, bigfood_y, bigfood_val     = cond
            self.agent.display_width = width
            self.agent.display_height = height
            self.env = SnakeEnv(snake_head_x, snake_head_y, food_x, food_y, width, height, rock_x, rock_y, bigfood_x, bigfood_y, bigfood_val)
            print('Testing init condition {}: snake_x: {} snake_y: {} food_x: {}, food_y: {}, '
                  'width: {}, height: {}, rock_x: {}, rock_y: {}, bigfood_x: {}, bigfood_y: {}, bigfood_val: {}'.format(
                i, snake_head_x, snake_head_y, food_x, food_y, width, height, rock_x, rock_y, bigfood_x, bigfood_y, bigfood_val)
            )
            points_results = []
    ...
    def show_games(self):
        ...
        self.env = SnakeEnv(self.args.snake_head_x, self.args.snake_head_y, self.args.food_x, self.args.food_y,
                            self.args.width, self.args.height, rock_x=self.args.rock_x, rock_y=self.args.rock_y)
        self.agent.display_width = self.args.width
        self.agent.display_height = self.args.height
        self.env.display()
        pygame.event.pump()
        self.agent.eval()
        points_results = []
...
def main():
    ...
    parser.add_argument('--bigfood_x', dest="bigfood_x", type=int, default=4,
                        help='initialized x position of bigfood  - default 4')
    parser.add_argument('--bigfood_y', dest="bigfood_y", type=int, default=5,
                        help='initialized y position of bigfood  - default 5')
    parser.add_argument('--bigfood_val', dest="bigfood_val", type=int, default=2,
                        help='value of bigfood  - default 2')
    ...
    parser.add_argument('--rock_x', dest='rock_x', type=int, default=11, help='x position of the rock - default 11')
    parser.add_argument('--rock_y', dest='rock_y', type=int, default=4, help='y position of the rock - default 4')

    args = parser.parse_args()
    app = Application(args)
    app.execute()

    # assert args.width == 18
    # assert args.height == 10
    # generate_discretization_test_data(app)
